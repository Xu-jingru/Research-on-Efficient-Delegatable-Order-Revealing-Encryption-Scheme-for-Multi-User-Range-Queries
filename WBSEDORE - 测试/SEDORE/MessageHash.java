...
public class MessageHash {
	...
	public void hash (Element[][] message_hash, int[] message, int messageLength) throws NoSuchAlgorithmException {
		...
		for (int i = 0; i < messageLength; i++) {
			...
			message_hash[i][0] = pairing.getZr().newElementFromHash(temp0Bytes, 0, temp0Bytes.length);
			message_hash[i][1] = pairing.getZr().newElementFromHash(temp1Bytes, 0, temp1Bytes.length);
		}
	}
}
