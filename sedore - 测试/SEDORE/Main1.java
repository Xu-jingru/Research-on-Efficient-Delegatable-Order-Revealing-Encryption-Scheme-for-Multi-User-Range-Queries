...
public class Main {
	...
	public static void main(String[] args) throws NoSuchAlgorithmException {
		...
		int[] m2 = converter.intToBinary(54, messageLength); //Message for v
		...
		for(int i=0; i<2000; i++) {
			test.test(GT, ct_u1[i], ct_v, tk_v_to_u, tk_u_to_v, messageLength);
//			if (result == 0)
...
//				System.out.println(i+":Test result: m1 < m2");
		...
		stime = System.currentTimeMillis();
		for(int i=0; i<2000; i++) {
			test.test(GT, ct_u2[i], ct_v, tk_v_to_u, tk_u_to_v, messageLength);
		}
		etime = System.currentTimeMillis();
		// 计算执行时间
		...
		stime = System.currentTimeMillis();
		for(int i=0; i<2000; i++) {
			test.test(GT, ct_u3[i], ct_v, tk_v_to_u, tk_u_to_v, messageLength);
		}
		etime = System.currentTimeMillis();
		// 计算执行时间
		...
		stime = System.currentTimeMillis();
		for(int i=0; i<2000; i++) {
			test.test(GT, ct_u4[i], ct_v, tk_v_to_u, tk_u_to_v, messageLength);
		}
		etime = System.currentTimeMillis();
		// 计算执行时间
		...
		stime = System.currentTimeMillis();
		for(int i=0; i<2000; i++) {
			test.test(GT, ct_u5[i], ct_v, tk_v_to_u, tk_u_to_v, messageLength);
		}
		etime = System.currentTimeMillis();
		// 计算执行时间
